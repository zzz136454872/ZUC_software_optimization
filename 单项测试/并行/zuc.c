...
/*the semaphore for mutex*/
//pthread_spinlock_t mutex, mutex2;
pthread_spinlock_t mutex2;
u8 ready = 1;
...
/* LFSR with work mode */ 
/*outLen is a int pointer that point to KeyStreamlen*/  
void* LFSRWorkThread(void *nothing) 
{ 
    u32 i, j;
    register u32 f, v;
    for(i=0;i<times;i++)
    {
		...
		/*update the state*/
		while (1){
			pthread_spin_lock(&mutex2);
			if (ready != 1) break;
			pthread_spin_unlock(&mutex2);
		}
        //while(ready==1);
        /* update the state */
        LFSR_S0 = LFSR_S1;
...
/*Bit Reorganization*/
void*  BRMultiThread(void* nothing)
{
	u32 i;
	for(i=0;i<times;i++)
	{
...
void GenerateKeystream(u32* pKeystream, int KeystreamLen)
{
	...
	pthread_t lfsrid,brid;	//the thread identifier 
	//pthread_spin_init(&mutex, 1);
	pthread_spin_init(&mutex2, 2);
	pthread_create(&lfsrid,NULL,LFSRWorkThread,NULL);
	//pthread_create(&brid,NULL,BRMultiThread,NULL);
	BRMultiThread(NULL);
	//LFSRWorkThread(NULL);
	int *nothing, *nothing2;
	pthread_join(lfsrid, (void**)&nothing2);
    //pthread_join(brid,(void**)&nothing);
    //pthread_spin_destroy(&mutex);
    pthread_spin_destroy(&mutex2);
}
