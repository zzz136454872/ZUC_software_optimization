...
{ 
	u32 i;
	long long t;
	t = ((long long)LFSR_S[15]<<15)+((long long)LFSR_S[13]<<17)+((long long)LFSR_S[10]<<21)+((long long)LFSR_S[4]<<20)+((long long)LFSR_S[0]<<8)+LFSR_S[0];
	t = (t&0x7FFFFFFF) + (t>>31);
	t = (t&0x7FFFFFFF) + (t>>31);
...
inline void GenerateCipherstream(u32 size, u32 *M,u32 *C)
{
	register u32 i;
	for (i=0; i<size; i++)
	{
